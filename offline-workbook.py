...
#        print("instance " + str(instance))
    ...
    for fileName in listaFiles:
        if fileName.endswith(".out") and fileName.startswith("awr"):
            ...
            if not data_os.empty:
                ...
                try:
                    physical_memory[db_name] = float(data_os.loc[data_os['STAT_NAME'] == 'PHYSICAL_MEMORY_GB'].iloc[0]['STAT_VALUE'])
                except:
    ...
    for dbname in main_metrics:
        ...
        peak_connections = pd.to_numeric(main_metrics[dbname]['aas_max']).max()
        ...
        for instance in range(1, (number_of_instances[dbname] + 1)):
            ...
            tittle = dbname + "-instance-" + str(instance_number)
            ...
            # I/O READ MB stats
            read_mb_s_max = pd.to_numeric(data_of_instance['read_mb_s_max'])
            ...
            # I/O WRITE MB stats
            write_mb_s_max = pd.to_numeric(data_of_instance['write_mb_s_max'])
