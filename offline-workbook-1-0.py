...
#        print("instance " + str(instance))
    ...
    for dbname in main_metrics:
        ...
        for instance in range(1, (number_of_instances[dbname] + 1)):
            ...
            data_of_instance.iloc[:,3:] = data_of_instance.iloc[:,3:].apply(convertToPoint)
            data_of_instance.iloc[:,3:] = data_of_instance.iloc[:,3:].apply(pd.to_numeric)

            # Set index as the "end" columns, that is the timestamp of each AWR sample
            data_of_instance = data_of_instance.set_index("end")


            #plotGraph(dataFrame, column, outFile, title, xLabel="Timestamp", yLabel="% CPU")
# PAREI AQUI
            # CPU stats
            os_cpu_max = pd.to_numeric(data_of_instance['os_cpu_max'].str.replace(',', '.'))
            plotGraph(os_cpu_max, graphsMainPath + "/cpu-max-" + title, title, "amostra", "% CPU máxima")
            plotHistogram(os_cpu_max, histMainPath + "/cpu-max-" + title, title, "% CPU máxima")
            ...
            if number_of_cpus[dbname] != '':
                ...
                plotGraph(db_cpu, graphsMainPath + "/db-cpu-" + title, title, "amostra", "% DB CPU")
                plotHistogram(db_cpu, histMainPath + "/db-cpu-" + title, title, "% DB CPU")

            # I/O READ MB stats
            read_mb_s_max = pd.to_numeric(data_of_instance['read_mb_s_max'].str.replace(',', '.'))
            plotGraph(read_mb_s_max, graphsMainPath + "/read-MB-max-" + title, title, "amostra", "Read MB máximo")
            plotHistogram(read_mb_s_max, histMainPath + "/read-MB-max-" + title, title, "Read MB máximo")

            # I/O READ IOPS Stats
            read_iops_max = pd.to_numeric(data_of_instance['read_iops_max'].str.replace(',', '.'))
            plotGraph(read_iops_max, graphsMainPath + "/read-IOPS-max-" + title, title, "amostra", "Read IOPS máximo")
            plotHistogram(read_iops_max, histMainPath + "/read-IOPS-max-" + title, title, "Read IOPS máximo")

            # I/O WRITE MB stats
            write_mb_s_max = pd.to_numeric(data_of_instance['write_mb_s_max'].str.replace(',', '.'))
            plotGraph(write_mb_s_max, graphsMainPath + "/write-MB-max-" + title, title, "amostra", "Write MB máximo")
            plotHistogram(write_mb_s_max, histMainPath + "/write-MB-max-" + title, title, "Write MB máximo")

            # I/O WRITE IOPS Stats
            write_iops_max = pd.to_numeric(data_of_instance['write_iops_max'].str.replace(',', '.'))
            plotGraph(write_iops_max, graphsMainPath + "/write-IOPS-max-" + title, title, "amostra", "Write IOPS máximo")
            plotHistogram(write_iops_max, histMainPath + "/write-IOPS-max-" + title, title, "Write IOPS máximo")

            # REDO LOG Write
            redo_mb_s = pd.to_numeric(data_of_instance['redo_mb_s'].str.replace(',', '.'))
            plotGraph(redo_mb_s, graphsMainPath + "/REDO-MB-per-sec-" + title, title, "amostra", "REDO MB/s")
            plotHistogram(redo_mb_s, histMainPath + "/REDO-MB-per-sec-" + title, title, "REDO MB/s")

            max_sessions = pd.to_numeric(data_of_instance['aas_max'].str.replace(',', '.'))
            plotGraph(max_sessions, graphsMainPath + "/Max-Sessions-" + title, title, "amostra", "Max Sessions")
            plotHistogram(max_sessions, histMainPath + "/Max-Sessions-" + title, title, "Max Sessions")

            # csv string
#            outString = outString + formatter(os_cpu_max.mean()) + ";" + formatter(os_cpu_max.max()) + ";"
            outString = outString + formatter(os_cpu_max.mean()) + ";"
            outString = outString + formatter(os_cpu_max.quantile(DEFAULT_PERCENTILE)) + ";" + formatter(os_cpu_max.max()) + ";"
            outString = outString + formatter(db_cpu.max()) + ";"
            outString = outString + formatter(db_cpu.quantile(DEFAULT_PERCENTILE)) + ";"
            ...
            outString = outString + formatter(redo_mb_s.mean()) + ";" + formatter(max_sessions.max()) + "\n"
            print(outString)
            ...
            cpu_utilization = numFormatter(os_cpu_max.quantile(DEFAULT_PERCENTILE))
            system_memory = physical_memory[dbname] if physical_memory[dbname] != '' else ''
            # Size for Maximum present memory
            memory_util = 100
            io_bandwdth = read_mb_s_max.max() + write_mb_s_max.max()
            ...
            io_growth = 10
            read_requests = numFormatter(read_iops_max.max())
            write_requests = numFormatter(write_iops_max.max())
            read_optimization = ''
            write_optimization = ''
            flash_seg_total = ''
            db_average_util = numFormatter(db_cpu.quantile(DEFAULT_PERCENTILE))
...
if __name__ == "__main__":
    ...
    if (len(sys.argv) != 2) or not os.path.exists(dirname + os.sep + sys.argv[1]):
        print("invalid arguments\n")
        sys.exit

    source_files = sys.argv[1]

#    createServersSheet(source_files)
