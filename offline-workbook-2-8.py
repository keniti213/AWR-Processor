#
# Version 2.8 29-Mar-2022
# 
# V2.7 - 29-03-2022 : a coluna cpu_per_s em Main Metrics mudou para cpu_p_sec
# V2.8 - 19-03-2022 : metodo append do Pandas ira ser descontinuado por isso, foi necessaria a troca por concat.
#
#
import yaml
...
# Build a Line corresponding to one of the DataFrames for ServerTemplate , DBTemplate or MappingTemplate
...
#        print("instance " + str(instance))
    ...
    # indexes used in the concat methods to build DataFrames by appending lines
    db_index = 0
    server_index = 0
    mapping_index = 0
    ...
    for dbname in main_metrics:
        ...
        df_line_to_append = pd.DataFrame(line_to_append, index=[db_index])
        db_df = pd.concat([db_df, df_line_to_append], axis=0)
        db_index = db_index + 1
        ...
        for instance in range(0, (number_of_instances[dbname])):
            ...
            if number_of_cpus[dbname] != '':
                data_of_instance["db_cpu"] = data_of_instance["cpu_p_sec"]/number_of_cpus[dbname]*100
                ...
                if cpu_count[dbname] != '':
                    data_of_instance["db_cpu"] = data_of_instance["cpu_p_sec"]/cpu_count[dbname]*100/number_of_instances[dbname]
                    plotGraph(data_of_instance, "db_cpu", graphsMainPath + "/db-cpu-" + title, title, "amostra", "% DB CPU")
                    plotHistogram(data_of_instance["db_cpu"], histMainPath + "/db-cpu-" + title, title, "% DB CPU")
                else:
...
#            outString = outString + formatter(os_cpu_max.mean()) + ";" + formatter(os_cpu_max.max()) + ";"
            ...
            df_line_to_append = pd.DataFrame(line_to_append, index=[server_index])
            servers_df = pd.concat([servers_df, df_line_to_append], axis=0)
            ...
            df_line_to_append = pd.DataFrame(line_to_append, index=[mapping_index])
            mapping_df = pd.concat([mapping_df, df_line_to_append], axis=0)
...
if __name__ == "__main__":
    ...
    if (len(sys.argv) == 2) and os.path.exists(dirname + os.sep + sys.argv[1]):
        ...
        print("invalid arguments: Offline-Workbook-2-8 <input files directory>\n")
        sys.exit()